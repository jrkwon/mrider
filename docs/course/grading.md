...
Collecting submissions, recording marks, and exporting for the gradebook are documented separately:
**[Grading Workflow](grading-workflow.md)**. Marks live in `course/scores-<term>-lab<N>.tsv` and
nowhere else — the per-student files under `dist/` are regenerated on every run.
