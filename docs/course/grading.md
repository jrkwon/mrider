...
```bash
# 1. Collect. Either source; the checks are identical.
bash scripts/lab.sh grade 1 --roster course/roster-2026-fall.txt   # from git
bash scripts/lab.sh grade 1 --zips ~/Downloads/lab1                # from the LMS

# 2. Open the marks file (created on first run, never overwritten).
bash scripts/lab.sh scores 1
#    -> course/scores-2026-fall-lab1.tsv

# 3. Read each report in dist/grade/lab1/<uniqname>.md, fill in
#    evidence (0-3) and understanding (0-2) in the marks file.

# 4. Total, apply the late policy, export.
bash scripts/lab.sh scores 1
#    -> dist/grade/lab1/final.csv
```

**Grade the archive, not the branch.** The LMS upload is the authoritative submission, so
`--zips` checks the artefact that actually counts; `--roster` is the faster read when you only want
to see who is stuck. The archive is cut from the commit with `git archive`, so the two agree unless
someone has gone out of their way.

Identity comes from **inside** the report, not the filename — the LMS prefixes student names and
appends `-1` to a second copy, and neither should invent a student or split one in two.

> [!WARNING]
> **Marks go in `course/scores-*.tsv` and nowhere else**
>
> `dist/grade/lab1/<uniqname>.md` is regenerated by every `grade` run, and `dist/` is git-ignored.
> A number typed there is lost twice over.
>
> The marks file is also git-ignored, deliberately: it is student grade data and this repository is
> public.
